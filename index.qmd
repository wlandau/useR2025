...
## Overview {style="font-size: 1.1em"}

<br>

Background: Bayesian Statistics in the life sciences
...
Traditional workflows in R struggle to scale.

. . .

<br>

Pipelines give you time, confidence, and peace of mind.
...
Novel therapy vs standard of care.
...
2. Draw a dataset given the parameters.
...
## Why this simulation validates the model {style="font-size: 80%"}

::: {.nonincremental}

<br>

SBC is equivalent to integrating out $\theta^{\text{sim}}$ and $y^{\text{sim}}$ to recover the prior:
...
$$
p(\theta | \theta^{\text{sim}}) = \int \color{blue}{p(\theta | y^{\text{sim}})} p(y^{\text{sim}} | \theta^{\text{sim}})  d y^{\text{sim}}
$$

. . . 

$$
p(\theta) = \int p(\theta | \theta^{\text{sim}}) d \theta^{\text{sim}}
$$
